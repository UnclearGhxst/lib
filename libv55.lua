...
sliderBackground.BackgroundTransparency = 0.18
    ...
    --// XSX Apple Glass v3 final polish pass
    -- Uses coroutine.wrap for executors where task.defer is unavailable.
    coroutine.wrap(function()
        for _ = 1, 100 do
            local root = CoreGuiService:FindFirstChild("XSX") or CoreGuiService:FindFirstChild("xsx")

            if root then
                for _, object in ipairs(root:GetDescendants()) do
                    if object:IsA("Frame") or object:IsA("ScrollingFrame") then
                        if object.BackgroundTransparency < 1 then
                            local large = object.AbsoluteSize.X > 260 and object.AbsoluteSize.Y > 100
                            if large then
